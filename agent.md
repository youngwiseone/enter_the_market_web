...
- Clicking an unlocked empty farm tile buys and plants the selected seed.
- Plants grow over days; clicking a fully grown plant harvests and sells it for 1.25× current market price.
...
- `purchaseGridSlot(index)`: unlocks a cell (price doubles each slot).
- `gridPlantedDay` determines growth progress.
- Growth stages:
...
- Harvest:
  - `harvestPlant(cellIndex)` sells at 1.25× price and clears the cell.
